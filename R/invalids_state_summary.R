...
#' @import tidyr
#' @export
invalids_state_summary <- function(data) {
  ...
  # clean up state invalids and return it
  return(
    state_invalids %>% # take invalids
      ...
      spread(C_Biosense_Facility_ID, value) # spread it back out by facility
  )
}
length(invalid_summaries)